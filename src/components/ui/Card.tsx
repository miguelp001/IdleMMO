...
const Card: React.FC<CardProps> = ({ 
  ...
  const paddingKey = padding as keyof typeof paddingClasses;

  const cardClasses = `${baseClasses} ${variantClasses[variantKey]} ${paddingClasses[paddingKey]} ${interactiveClasses} ${className}`;
...
export const CardHeader: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <div className={`px-6 py-4 border-b border-gray-800 ${className}`}>{children}</div>
);
